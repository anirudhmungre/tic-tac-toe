<template>
  <div class="text-5xl flex flex-col items-center m-4">
    <h1 class="font-bold m-2 text-6xl">
      TIC-TAC-TOE
      <button :class="{ 'animate-pulse': gameStore.gameOver }" @click="reset">
        🔄
      </button>
    </h1>
    <div
      class="grid grid-cols-3 gap-4 place-items-center w-fit bg-teal-800 rounded-lg p-4 m-4"
    >
      <button
    ...
    <h2 v-if="!gameStore.gameOver">
      It is {{ getCurrentPlayer() }}'s turn!
      {{ getBoxIcon(gameStore.currentPlayer) }}
    </h2>
  ...
  <div
    class="grid grid-cols-2 w-max bg-slate-700 rounded-xl text-5xl text-left p-4 justify-self-center"
  >
    ...
    <h2>{{ playerStore.p2.name }}</h2>
    <h2 class="text-right">{{ playerStore.p2.score }}</h2>
  </div>
</template>
